...
from kmers_for_component_p11 import kmers_for_component
...
if run_jellyfish or run_extension_corr:
    run_cmd('pypy kp1mer_to_kmer.py ' + sample_name_input+'algo_input/k1mer.dict ' + sample_name_input+'algo_input/kmer.dict')
...
'''if run_remaining_comps:
    ...
    run_cmd('pypy kp1mer_to_kmer.py ' + base_dir+sample_name+'remaining_algo_input/k1mer.dict ' + base_dir+sample_name+'remaining_algo_input/kmer.dict')
...
if double_stranded:
    run_cmd("parallel python main_server_WWY_p11.py {} --run_alg --ds --kmer_size " + str(K)  + " " + paired_end_flag + " ::: " + main_server_parameter_string)
else:
    run_cmd("parallel python main_server_WWY_p11.py {} --run_alg --kmer_size " + str(K) + " " + paired_end_flag + " ::: " + main_server_parameter_string)
...
dir = dir_base + "algo_output"
run_cmd("mkdir " + dir)
temp_file = dir + "/" + "all_reconstructed.fasta"
temp_file_args = "" 
for comp in reconstructed_files:    
    for file in reconstructed_files[comp]:
        temp_file_args = temp_file_args + file + " "
...
process_concatenated_fasta(temp_file, dir + "/reconstructed.fasta")
run_cmd("cp " + og_algo_output + "/reference.fasta " + dir + "/")
run_cmd("python main_server_WWY_p11.py " + dir_base + " --compare ")
