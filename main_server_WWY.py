...
if paired_end:
	...
	if fastq:
		reads_string = reads_file + '_1.fastq '+reads_file + '_2.fastq '
else:	
	reads_string = reads_file + '.fasta '
	if fastq:
		reads_string = reads_file + '.fastq'
...
if compare_soap:
	...
	genome_index = './Genome/hg19 '
        ...
        soap_fasta = './' +sample_output_name+'algo_output/SOAP/SOAP.contig'
	...
	long_ref =  './' +sample_name+'algo_input/reference_long.fasta'
	soap_long_per =  './' +sample_output_name+'algo_output/SOAP/soap_long_per.txt'	
...
if compare_soap:
	if sensitivity:
        ...
        if false_positive:
		run_cmd('python parallel_blat.py ' + soap_fasta + ' ' + long_ref + ' ' + soap_long_per)
                tester.false_positive(soap_fasta,soap_long_per,soap_rev_log)
...
if run_oasis or compare_oasis:
	...
	long_ref = './' +sample_name+'algo_input/reference_long.fasta'
        ...
        oasis_fasta = './' +sample_name+'algo_output/OASIS/contigs.fa'
        oasis_log = './' +sample_name+'algo_output/OASIS/oasis_log.txt'
	oasis_rev_per = './' +sample_name+'algo_output/OASIS/oasis_rev_per.txt'
...
if compare_oasis:
	if sensitivity:
        ...
        if false_positive:
                run_cmd('python parallel_blat.py ' + oasis_fasta + ' ' + long_ref + ' ' + oasis_rev_per)
		tester.false_positive(oasis_fasta,oasis_rev_per,oasis_rev_log)
...
if compare_trans:
	...
	trans_rev_per = './' +sample_name+'algo_output/TRANS/trans_rev_per.txt'
...
if compare_trans:
	if sensitivity:
        ...
        if false_positive:
		run_cmd('python parallel_blat.py ' + trans_fasta + ' ' + long_ref + ' ' + trans_rev_per)
                tester.false_positive(trans_fasta,trans_rev_per,trans_rev_log)
...
if plots_express:
	...
	num_isoform_file = './' +sample_output_name+'algo_output/kmer_oracle.isoform'
        plotOthers_file = './' +sample_output_name+'algo_output/plots_TrOaTR_isoGeq5_EXP.txt'
	plotMain_file = './' +sample_name+'algo_output/plots_ShTrTr_isoGeq5_EXP.txt'
	...
	if sim:
		...
		#tester.performance_plot_express(oracle_fasta,exp_file,oasis_log,soap_log,trans_log,plot_file,exp_file,'SIM',L,200,N,iso_low,iso_high,use_oracle)
                ...
                tester.performance_plot_express(oracle_fasta,num_isoform_file,reconstr_log,trinity_log,trinity_log,plotMain_file,express_file,'EXP',L,200,N,iso_low,iso_high,use_oracle)
		...
		tester.performance_plot_express(oracle_fasta,num_isoform_file,trinity_log,soap_log,trans_log,plotOthers_file,express_file,'EXP',L,200,N,iso_low,iso_high,use_oracle)
